...
def parse(p):
    ...
    return d
dob = parse(DOBIE); en = parse(STOCK+'/en-us.cfg'); ja = parse(STOCK+'/ja.cfg'); zh = parse(STOCK+'/zh-cn.cfg')
...
print('K_dobie where ko value != en-us value:', len(diff))
...
T = common - K
print()
print('--- zh kept EN but Dobie translated: %d (sample, zh-only) ---' % len(zh_keep&T))
for k in sorted((zh_keep-ja_keep)&T)[:20]:
    print(' ',k,'| en:',en[k][:55],'| ja:',ja[k][:35],'| ko:',dob[k][:35])
print()
print('--- ja kept EN but Dobie translated: %d (all) ---' % len(ja_keep&T))
...
print('K_dobie key prefixes:', c2.most_common(12))
