...
구 도구에서 Squad 기계번역 시에도 CRP와 같은 프리즈가 발생했고(그래서 `EXCLUDED_DIRS`에 Squad/SquadExpansion 포함), 사람이 번역한 구 커뮤니티 패치는 문제가 없었다.

**검증 방법**: KSP 1.12.5 스톡 원본 dictionary.cfg(en-us/ja/zh-cn)를 미러 레포([Apofoo/KSP-Localization](https://github.com/Apofoo/KSP-Localization), Epic판)에서 확보. 키 개수 완전 일치(11,932) 및 구 커뮤니티 패치 영어 잔존 값의 94.9%가 en-us와 문자 단위 일치함으로 원본·베이스 관계 확정.

**결과 (공통키 11,930 기준)**:

- 구 패치가 영어로 남긴 키: 3,492개(29.3%). 그러나 공식 ja는 199개(1.7%), zh-cn은 364개(3.1%)만 영어 유지 — **구 커뮤니티 패치 영어 잔존분의 94.6%를 공식 번역은 정상 번역했다** (크레딧 롤, 파트 플레이버 텍스트, 계약 문구 등 — 즉 구 커뮤니티 패치의 영어 유지는 안전 조치가 아니라 **미번역 영역**이었다).
...
1. 프리즈 방지의 주 방어선은 **토큰/구조 검증** (구 패치가 안전했던 실제 이유는 `<<1>>` 2,896건·`^` 49건 완전 보존으로 추정).
2. **역방향 규칙**: ja가 번역한 키(98.3%)는 안심 번역 대상. 구 커뮤니티 패치 영어 잔존 3,492개 중 3,329개(95.3%)를 "번역 필요"로 정확히 분류 — 작업 우선순위 산정에 활용.
3. ja 영어 유지 키는 검토 큐: 단위/포맷류(~160건)는 자동 영어 유지, UI 축약어(`OK`→확인, `VAB`→조립동 등 36건)는 구 커뮤니티 패치처럼 한국어화.
4. ja 번역문을 LLM 참조 힌트로 주입 (zh-cn은 보조).
5. 제품명·실존 고유명사(`Steam`, `KSPedia`, `Rosetta` 등)는 구 패치 음차 대신 원문 유지 재검토.

부가 사실: 스톡 원본 en-us/ja/zh-cn이 확보되어(scratchpad) Squad 작업에는 Steam 복원 없이도 마스크·참조 데이터를 쓸 수 있다 (게임 무결성을 위한 복원은 별개로 필요).

- 구 커뮤니티 패치 번역(사람 번역, 검증된 품질)은 Squad ko 팩의 **초기 변형(variant) 시드로 재활용 가치가 큼** — 커버리지 ~70%(미번역 29.3% 제외). 단, 원작자 허락/크레딧 협의 필요 (미확정 항목).

### 현재 설치본 오염 상태

- `GameData/Squad/Localization/dictionary.cfg`: 커뮤니티 패치(24.06.15)로 **파일 전체가 교체**되어 en-us 단일 노드(값은 한국어)만 남음 — 다른 8개 언어 노드(ja, zh-cn 등)가 로컬에서 소실. `SquadExpansion/Serenity/Localization/dictionary.cfg`(73KB)도 동일하게 en-us 단일 노드로 교체됨. 공식 번역 마스크 추출에는 Steam 무결성 검사로 원본 복원이 선행되어야 한다.
...
### 검증으로 해소 예정 (스파이크 대상) — 결과: 부록 F (2026-08-16 실게임 검증)
- [x] `language = ko`에서 메뉴 폰트가 실제로 매칭되는가 → **확인됨** (메뉴 폰트가 내장 한국어 폰트로 전환, 렌더링 정상)
...
- 구 커뮤니티 패치 번역의 Squad 초기 변형 시드 재활용 — 원작자 허락/크레딧 협의 필요
