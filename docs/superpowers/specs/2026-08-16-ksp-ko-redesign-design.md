...
### CRP류 로딩 프리즈의 원인 구조 (사용자 피드백 2 반영)

`CommunityResourcePack/Localization/en-us.cfg`를 확인한 결과, DisplayName 태그의 값이 내부 자원 식별자와 **동일한 문자열**이다 (`#LOC_CRP_Food_DisplayName = Food` 등). 일부 모드/코드는 자원을 표시 이름 문자열로 조회·비교하므로, 이 값을 번역하면 매칭이 깨져 로딩 프리즈 등이 발생한다.

**중요**: 이 문제는 en-US 교체 방식뿐 아니라 새 추가 방식(ko 노드가 태그 값을 덮어씀)에서도 동일하게 발생한다. → 블랙리스트는 새 설계의 필수 요소이며, 개인 설정이 아니라 **DB에서 공유되는 데이터**여야 한다 (아래 DB 구조의 `blacklist.json`).

- 블랙리스트 수준: 모드 단위(예: CommunityResourcePack 전체) + 키 단위(특정 태그) + 패턴 단위(`*_DisplayName` 중 값이 식별자 형태인 것)
- 보조 휴리스틱(기존 classifier 계승): 값이 식별자처럼 생긴 경우(공백 없음, CamelCase, 원문=키 유사) 자동 SKIP
- CI가 팩 검증 시 블랙리스트 위반을 거부
...
5. 실모드 3~4개(태그형/비태그형/블랙리스트 대상 혼합 — CRP 포함)로 E2E: 팩 생성 → 설치 → 게임 확인 → 제거.
