...
### Squad(스톡) 프리즈도 같은 부류임을 확인 (2026-08-16 추가 분석)

구 도구에서 Squad 기계번역 시에도 CRP와 같은 프리즈가 발생했고(그래서 `EXCLUDED_DIRS`에 Squad/SquadExpansion 포함), 사람이 번역한 Dobie 패치는 문제가 없었다. 현 설치본의 Dobie 번역 dictionary.cfg를 분석한 결과:

- Lingoona 치환 토큰 `<<1>>` 포함 2,896줄이 전부 보존됨, 성별 토큰 `^`도 49건 유지
- 전체 12,041줄 중 **3,511개 항목(~29%)을 의도적으로 영어 유지** — 단위(`m/s`), 키 이름(`Tab`, `Track IR`), 포맷 전용 문자열(`<<1>>x<<2>>`) 등

→ 기계번역 프리즈의 원인 부류가 CRP와 동일함을 시사: 포맷/단위/식별자 문자열까지 번역하거나 토큰을 훼손하면 네이티브 Lingoona(`Grammar.useGrammar`)나 소비 코드가 깨진다. **해법도 동일**: 스톡 dictionary.cfg(복원 후)에는 ja/zh-cn 공식 노드가 있으므로 공식 번역 마스크가 Squad에 그대로 적용되고(일본어판이 영어 유지한 키 = 우리도 유지), 최종 검증의 토큰 보존 검사가 이중 방어선. 정확한 소비자 코드 특정은 CRP와 함께 스파이크 재현 항목.

- Dobie 번역(사람 번역, 검증된 품질)은 Squad ko 팩의 **초기 변형(variant) 시드로 재활용 가치가 큼** — 단, 원작자 허락/크레딧 협의 필요 (미확정 항목).
...
2. **병렬 번역**
   - 공식 번역 마스크/블랙리스트/휴리스틱으로 대상 키 확정 → 배치 병렬 번역.
...
- Squad에서 공식 번역(ja/zh-cn)이 영어 유지한 키 목록 — 스톡 복원 후 마스크 추출로 확정
