...
| c | 미번역 키 영어 폴백 | **1차 판정 불가** (구 커뮤니티 패치 잔재로 전체가 한국어 표시) → 스파이크 후 Squad·Serenity dictionary.cfg를 스톡 원본(미러)으로 복원 완료, **재실행 검증 대기** |
| d | 로딩 정상(프리즈 없음) | **성공** |
...
- Squad `dictionary.cfg`: 구 커뮤니티 패치(1.0MB, en-us 단일 노드 한국어) → 스톡 미러 en-us+ja+zh-cn 병합본(2.97MB)으로 교체
